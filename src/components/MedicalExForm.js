...
const MedicalExForm = ({ selectedPatientId, onPrescriptionChange }) => {
  ...
  const handleRemove = (prescriptionId) => {
    const updatedPrescriptions = prescriptionIds.filter((item) => item !== prescriptionId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await beAxios.post(`/medical-records/${selectedPatientId}`, data);
      if (res && res.data) {
          ...
          <div className="input-container">
            ...
            <div className="form-right">
                  ...
                  <ul>
                    {prescriptionIds.map((prescriptionId) => (
                      <li key={prescriptionId} className="me-2">
                        {prescriptions.find((p) => p.id === prescriptionId).name}{' '}
                        <i className="pi pi-times" onClick={() => handleRemove(prescriptionId)} />
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <div className="input-field">
                ...
                <br />
                <InputTextarea placeholder="Chuẩn đoán" rows={5} cols={30} onChange={handleDescriptionChange} />
              </div>
            </div>
          </div>
